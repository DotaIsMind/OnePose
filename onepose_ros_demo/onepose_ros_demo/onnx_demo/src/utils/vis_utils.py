...
def draw_pose_axes(image, pose_homo, K, axis_length=0.05, linewidth=3):
    """
    Draw X/Y/Z coordinate axes of the object pose onto the image.

    The three axes are projected from 3D object space to 2D image space
    using the given camera intrinsics and pose:
      - X axis → Red
      - Y axis → Green
      - Z axis → Blue

    @param image:       BGR image (H, W, 3) to draw on (modified in-place)
    @param pose_homo:   [4, 4] homogeneous camera-from-object pose matrix
    @param K:           [3, 3] camera intrinsic matrix
    @param axis_length: length of each axis arrow in metres (default 0.05 m)
    @param linewidth:   thickness of the drawn lines in pixels
    """
    if pose_homo is None:
        return image

    # Ensure numpy arrays
    pose_homo = np.array(pose_homo)
    K = np.array(K)

    # Origin and three axis endpoints in object space
    origin_3d = np.array([[0.0, 0.0, 0.0]])
    x_end_3d  = np.array([[axis_length, 0.0,         0.0        ]])
    y_end_3d  = np.array([[0.0,         axis_length,  0.0        ]])
...
def save_demo_image(
    ...
    Optionally overlays the object coordinate axes derived from *pose_homo*
    (the full 4×4 homogeneous pose matrix).  When *pose_homo* is provided and
    *draw_axes* is True, three colour-coded arrows are drawn at the object
    origin:
      - X axis → Red
      - Y axis → Green
      - Z axis → Blue

    @param pose_pred:    [3,4] or [4,4] pose used for 3-D box reprojection
    @param K:            [3,3] camera intrinsic matrix
    @param image_path:   path to the source image file
    @param box3d_path:   path to the 3-D bounding-box corner file
    @param draw_box:     whether to draw the reprojected 3-D bounding box
    @param save_path:    where to write the output image (None = skip saving)
    @param pose_homo:    [4,4] homogeneous pose for axis visualisation;
                         falls back to *pose_pred* when None
    @param draw_axes:    whether to draw the coordinate-axis arrows
    @param axes_length:  length of each axis arrow in metres (default 0.05 m)
    """
    box3d = np.loadtxt(box3d_path)
    ...
    if draw_axes:
        ...
        _pose_for_axes = pose_homo if pose_homo is not None else pose_pred
        draw_pose_axes(image_full, _pose_for_axes, K, axis_length=axes_length)
